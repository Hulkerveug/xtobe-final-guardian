...
export default function RetroCRT({ onEnterCompanion }: { onEnterCompanion?: () => void }) {
  ...
  return <main className="retro-crt" aria-label="Xtobe AI Guardian console"><Particles /><div className="crt-scanlines" /><header className="crt-header"><span>XTOBE-AI // FINAL GUARDIAN 2.0.0</span><span>● TAURI CONNECTED</span><button onClick={() => setMuted((value) => !value)}>{muted ? "SOUND OFF" : "SOUND ON"}</button><button onClick={replay}>REPLAY INTRO</button></header>{view === "landing" && <section className="crt-content"><p className="crt-kicker">GUARDIAN OF PC • PURE DARK ENERGY</p><h1 className="crt-title" data-text="XTOBE-AI">XTOBE-AI</h1><p className="crt-lore">THE FIRE DOES NOT CONSUME HIM. IT FORGES HIM.</p><section className="crt-tactical"><span>VIBRANIUM CORE • v2.7.1</span><strong>1.3°S 30.5°E • CLOAK {awake ? "DISENGAGED" : "ENGAGED"}</strong><i>{awake ? "CORE ONLINE" : "AWAITING AUTHORIZATION"}</i><b>TOKEN LEDGER: {balance ? balance.balance : "—"}</b></section><button className="crt-activate" onClick={activate}>ACTIVATE GUARDIAN</button></section>}{view === "cinematic" && <section className="crt-cinematic"><div className="crt-progress">{[0, 1, 2, 3].map((item) => <i className={item === slide ? "active" : ""} key={item} />)}</div><h2>{["ENTERING NO SIGNAL WAKANDA", "XTOBE-AI AWAKENING", "GUARDIAN OF THE THRONE", "ENTERING WAKANDA"][slide]}</h2><p>{["WHITE NOISE FLASH • SECURE LINK", "VIBRANIUM MASK • PROTOCOL 7", "NOT DEAD • VISUAL THEME ONLY", "LIGHTSPEED WARPED REALM"][slide]}</p>{slide === 0 && <div className="crt-boot-log"><div>BOOTSTRAP_LINK ........... {bootReady ? "OK" : "CHECKING"}</div><div>PROCESS_MATRIX .......... {bootReady ? "READY" : "CHECKING"}</div><div>GUARDIAN_CORE ........... {bootReady ? "OK" : "CHECKING"}</div><div>VIBRANIUM_GATE .......... {bootReady ? "READY" : "CHECKING"}</div></div>}{bootReady && <span className="crt-system-ready">SYSTEM READY</span>}<button onClick={() => setView("chat")}>SKIP INTRO →</button></section>}{view === "chat" && <section className="crt-chat"><div className="crt-messages">{messages.map((message, index) => <p className={message.role} key={`${message.role}-${index}`}><b>{message.role === "guardian" ? "XTOBE GUARDIAN" : "YOU • SEEKER"}</b>{message.text}</p>)}</div><div className="crt-dashboard"><div><span>THREAT STATUS</span><b className="crt-cyan">{processes.length ? `${processes.filter((item) => item.risk !== "safe").length} FLAGGED` : "READY"}</b></div><div><span>VIBRANIUM SHIELD</span><b className="crt-amber">{awake ? "ACTIVE" : "STANDBY"}</b></div><button onClick={runScan} disabled={scanning}>{scanning ? "SCANNING..." : "RUN DEEP SCAN"}</button></div><div className="crt-actions"><button onClick={() => note("SCAN THREATS: use the standard Threat Matrix for a read-only process inspection.")}>SCAN THREATS</button>{onEnterCompanion && <button onClick={onEnterCompanion}>COMPANION VAULT</button>}<button onClick={activate}>{awake ? "CLOAK ONLINE" : "CLOAK PC"}</button><button onClick={() => note("ANCESTRAL LOG: local Guardian history is available in the standard dashboard.")}>ANCESTRAL LOG</button></div><div className="crt-input"><input value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={(event) => event.key === "Enter" && send()} placeholder="Speak to the Guardian..." /><button onClick={send}>SEND</button></div><div className="crt-wave">{Array.from({ length: 32 }, (_, index) => <i key={index} />)}</div></section>}<footer className="crt-footer">LOCAL TERMINAL / NO CLOUD UPLINK / FACTUAL SECURITY OUTPUT</footer></main>;
}
